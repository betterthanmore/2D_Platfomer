...
public class SubPlayer : PlayerMainController
{
    ...
    public bool maxDistance = true;            //부스트로 올라간 거리가 최대치가 됐을 때 자동으로 떨어지게

    public override void Start()
    {

        base.Start();
        Debug.Log("서브" + playertype);
        Debug.Log("서브" + HorizontalKeyBoard);
    ...
    public override void Update()
    {
        ...
        if (GameManager.move)
        {
            ...
            if (gameObject.tag == "SubPlayer" && Input.GetButton(JumpKeyMap) && scrollbar.size > 0.001 && maxDistance && enableBoost || Input.GetKey(KeyCode.UpArrow) && scrollbar.size > 0.001 && maxDistance && enableBoost)        //부스트
            {
