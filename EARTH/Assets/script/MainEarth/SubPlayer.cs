...
public class SubPlayer : PlayerMainController
{
    ...
    private float subplayerPosY = 0;    //부스트 사용할 때의 플레이어의 위치 저장
    ...
    {
        base.Update();
        if (Input.GetKeyDown(KeyCode.W) && enableBoost)     //부스트 키를 눌렀을 때
        {
            subplayerPosY = gameObject.transform.position.y;
        }
        if (gameObject.tag == "SubPlayer" && Input.GetKey(KeyCode.W) && rb.velocity.y <= boostDistanceLimit && scr.value > 0.001 && enableBoost)        //부스트
        {
